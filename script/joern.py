...
def run_joern(filepath: str, verbose: int):
    script_file = gcn.script_dir() / "get_func_graph.scala"
    filename = gcn.script_dir() / filepath
    # print(filename)
    # return
    params = f"filename={filename}"
    command = f"joern --script {script_file} --params='{params}'"
    command = str(gcn.script_dir() / "joern-cli" / command)
    # print(command)
    # return 
    if verbose > 2:
        gcn.debug(command)
    ...
    try:
        shutil.rmtree(gcn.external_dir() / "joern-cli" / "workspace" / filename.name)
    except Exception as E:
        if verbose > 4:
            print(E)
        pass
