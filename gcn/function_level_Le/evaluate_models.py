...
def main(granularity, scope, cvss_col, feature):
	...
	cvss2_severity_dict = {"LOW":0,"MEDIUM":1,"HIGH":2}

	df["cvss2_severity"] = df[["cvss2_severity"]].apply(
	lambda r: cvss2_severity_dict[r.cvss2_severity], axis=1
	)
	cvss2_AV_dict = {"N":0,"L":1,"A":2}
	df["cvss2_AV"] = df[["cvss2_AV"]].apply(
	lambda r: cvss2_AV_dict[r.cvss2_AV], axis=1
	)
	cvss2_AC_dict = {"L":0,"M":1,"H":2}
	df["cvss2_AC"] = df[["cvss2_AC"]].apply(
		lambda r: cvss2_AC_dict[r.cvss2_AC], axis=1
	)
	cvss2_AU_dict = {"S":0,"N":1}
	df["cvss2_AU"] = df[["cvss2_AU"]].apply(
		lambda r: cvss2_AU_dict[r.cvss2_AU], axis=1
	)
	cvss2_C_dict = {"P":0,"N":1,"C":2}
	df["cvss2_C"] = df[["cvss2_C"]].apply(
		lambda r: cvss2_C_dict[r.cvss2_C], axis=1
	)
	cvss2_I_dict = {"P":0,"N":1,"C":2}
	df["cvss2_I"] = df[["cvss2_I"]].apply(
		lambda r: cvss2_I_dict[r.cvss2_I], axis=1
	)
	cvss2_A_dict = {"P":0,"N":1,"C":2}
	df["cvss2_A"] = df[["cvss2_A"]].apply(
		lambda r: cvss2_A_dict[r.cvss2_A], axis=1
	)

	df["cvss2_severity"] = df["cvss2_severity"].astype("int")
	df["cvss2_AV"] = df["cvss2_AV"].astype("int")
	df["cvss2_AC"] = df["cvss2_AC"].astype("int")
	df["cvss2_AU"] = df["cvss2_AU"].astype("int")
	df["cvss2_C"] = df["cvss2_C"].astype("int")
	df["cvss2_I"] = df["cvss2_I"].astype("int")
	df["cvss2_A"] = df["cvss2_A"].astype("int")
