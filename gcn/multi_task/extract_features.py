...
def model_output(model,input:str):
    codebert_tokens = [[int(i) for i in input.split()]]
    codebert_tokens = torch.tensor(codebert_tokens).int()
    codebert_attention_masks = (codebert_tokens!=1).int()
    
    _,output = model(codebert_tokens,codebert_attention_masks) 
    # print(output.shape)
    list =[str(round(i,4)) for i in output[0].tolist()]
    return " ".join(list)
