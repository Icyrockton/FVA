...
from model import GModel_Embedding_MultiTask, EarlyStopping
...
gnn_type = "gcn" #gatv2  gcn
codebert_type = 'codebert-base' #  codebert-base unixcoder-base-nine
codebert_type_name = codebert_type.split('-')[0]
save_path = f"./fasttext_result/{gnn_type}/{context_type}"
model_name = f"{codebert_type_name}_finetune"
# context_type_dict = {
#     'nature' : 'nature_context__codebert_tokens' ,
#     'data_flow' : 'DD_context_codebert_tokens' ,
#     'control_flow' : 'CD_context_codebert_tokens',
#     'sentence' : 'sentence_codebert_tokens'
# }

context_type_dict = {
    'nature' : 'nature_feature' ,
    'data_flow' : 'data_feature' ,
    'control_flow' : 'control_feature',
...
def get_metrics(true_dict, prob_dict,print_metrics:bool = False):
    ...
    for task_type in true_dict.keys():
        ...
        metrics_item["precision"] = round(precision_score(true, pred, average='macro', zero_division=1), 3)
...
def train(
        ...
        model: Union[GModel_Embedding_MultiTask],
    ...
    for batch in dl:
        ...
        tokens_1gram = g.ndata.pop("tokens_1gram").to(dev)
        tokens_2gram = g.ndata.pop("tokens_2gram").to(dev)
        tokens_3gram = g.ndata.pop("tokens_3gram").to(dev)
        ...
        pre_item = model(g, (tokens_1gram,tokens_2gram,tokens_3gram))    # (batch_size,dict)
        optimizer.zero_grad()
        ...
        del g
        del tokens_1gram , tokens_2gram ,tokens_3gram
        del loss
...
def main(lr="Adam", lr_rate=0.0001, dropout=0.1):
    ...
    log_info(f'learning_rate: {lr_rate} device:{os.environ["CUDA_VISIBLE_DEVICES"]} context_type:{context_type} sentencefirst:{sentence_first},early_stop:{early_stop_metric}')
    train_ds = GraphDataset_Token_MultiTask(context_type = context_type_dict[context_type] , token_type = 'fasttext' , partition="train")
    val_ds = GraphDataset_Token_MultiTask(context_type = context_type_dict[context_type] , token_type = 'fasttext' ,partition="valid")
    test_ds = GraphDataset_Token_MultiTask(context_type = context_type_dict[context_type] , token_type = 'fasttext' ,partition="test")
    # train_ds.cache_features()
    # val_ds.cache_features()
    # test_ds.cache_features()
    # return 
    # print(train_ds[0][0].ndata["codebert_tokens"].shape)
    # print(train_ds[0][0].ndata["codebert_tokens"])
    dl_args1 = {"drop_last": False, "shuffle": True, "num_workers": 6}
    dl_args2 = {"drop_last": False, "shuffle": False, "num_workers": 6}
    train_dl = GraphDataLoader(train_ds, batch_size=128, **dl_args1)
    val_dl = GraphDataLoader(val_ds, batch_size=128, **dl_args2)
    test_dl = GraphDataLoader(test_ds, batch_size=128, **dl_args2)
    ...
    model = GModel_Embedding_MultiTask(gnn_type,  dropout=dropout)
    ...
    early_stopping = EarlyStopping(save_path, model_name=model_name, verbose=True,patience=15)
